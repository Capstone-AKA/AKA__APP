...
const ENTRY_RSSI_THRESHOLD = -85; // -80 → 너무 빡셌음
...
export default function StoreBLE() {
  ...
  useEffect(() => {
    ...
    if (
      ...
      entry.rssi > ENTRY_RSSI_THRESHOLD &&
      !entryDetected
    ) {
      console.log(`🚪 ENTRY 감지됨! RSSI: ${entry.rssi}`);
  ...
  const handleCartRegister = async () => {
    ...
    if (isNaN(parsed) || parsed <= 0)
      return Alert.alert("알림", "올바른 카트 번호를 입력해주세요.");
    ...
    try {
      if (!USE_MOCK) {
        ...
        if (!res.data.success) {
          return Alert.alert("등록 실패", res.data.message || "카트 번호 오류");
        }
      }
      ...
      console.log("📡 카트 등록 → BLE 스캔 시작");
      startScan();
    } catch (e) {
      console.error("카트 등록 오류:", e);
      Alert.alert("오류", "카트 등록 중 문제가 발생했습니다.");
    }
  };
