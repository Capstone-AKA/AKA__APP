...
export default function StoreBLE() {
  ...
  function handleEntry(device: any, stopScan: () => void) {
    if (!cartRegistered) return;

    if (device?.rssi > ENTRY_RSSI_THRESHOLD) {
      console.log("🚪 ENTRY 비콘 감지!");
      setStoreId(1);
      stopScan();
      router.replace("/cart");
    }
  ...
  const { startScan, stopScan } = useBLE({
    onEntryDetected: (device) => handleEntry(device, stopScan),
  });
